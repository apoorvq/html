...
if (isset($_GET['category'])) 
{
    ...
    if (($category != 'Apparel') && ($category != 'Textile') && ($category != 'Fashion') && ($category != 'Technical textile')&& ($category != 'Technology And Innovation') && ($category != 'Corporate')  && ($category != 'Events') && ($category != 'Retail') && ($category != 'E-Commerce') && ($category != 'Institutional') && ($category != 'Denim')) 
    {
        ...
        echo '<h1> This is not a Category</h1>';
    }
    else { 
        ...
        $stmt = $conn->prepare("SELECT pk_i_id from s_categories where categories_name = :category ");
        $stmt->bindparam(":category", $category);
        $stmt->execute();
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        $categoryName = $result['pk_i_id'];
        $news = getNewsByCategory($categoryName);
        $event = getEventByCategory($categoryName);
        print_r($events);
        ...
        <div class="auto-container">
            ...
            <div class="shop-items">
                <div class="row clearfix">
                    ...
                    <!--Shop Item-->
                    <?php //echo $news['sources'][0];
                     for ($i=0; $i < 16 ; $i++) {  ?>
                    <div class="shop-item col-lg-3 col-md-4 col-sm-6 col-xs-12">
                        <div class="inner-box">
                            <div class="off-price"><?php echo $news['cardDate'][$i]; ?></div>
                            <div class="image-box">
                                <a href="singlenews.php?news=<?php echo $news['slugs'][$i];?>"><img width="277px" height="333px" src="<?php echo $news['sources'][$i]; ?> alt="" /></a>
                            </div>
                            <div class="lower-box">
                                <div class="upper-box">
                                    <h4 style="height: 100px"><a href="singlenews.php?news=<?php echo $news['slugs'][$i];?>"><?php echo $news['headlines'][$i]; ?></a></h4>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="auto-container">
            ...
            <div class="shop-items">
                <div class="row clearfix">
                    ...
                    <!--Shop Item-->
                    <?php for ($i=0; $i < $event['count'] ; $i++) {  ?>
                    <div class="shop-item">
                        <div class="inner-box">
                            <div class="image-box">
                                <a href="singlenews.php?news=<?php echo $news['slugs'][$i];?>"><img width="277px" height="333px" src="<?php echo $event['source'][$i]; ?>" alt="" /></a>
                            </div>
                            <div class="lower-box">
                                <div class="upper-box">
                                    <h4 style="height: 90px"><a href="singlenews.php?news=<?php echo $news['slugs'][$i];?>"><?php echo $event['event'][$i]." in ".$event['city'][$i]." ,".$event['country'][$i]; ?></a></h4>
                                </div>
                                <div class="lower-content">
                                    <div class="price"><?php echo "From ".$event['dateStart'][$i]." to ".$event['dateEnd'][$i]; ?></div>
                                </div>
                            </div>
                        </div>
                    </div>
